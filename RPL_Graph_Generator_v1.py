...
def change_converter():
    ...
    if _converter:
        ...
        root.submenu.entryconfig(2, state=DISABLED)
    else:
        _converter = True
        _debug("Converter On")
        root.submenu.entryconfig(1, label='Turn Converter Off')
        root.submenu.entryconfig(2, state=NORMAL)


def change_export_type():
    global _OutPutFileType
    if _OutPutFileType == 'png':
        _OutPutFileType = 'pdf'
        _debug(f"Output File Type = {_OutPutFileType.upper()}")
        root.submenu.entryconfig(2, label='Export to PNG')
    elif _OutPutFileType == 'pdf':
        _OutPutFileType = 'png'
        _debug(f"Output File Type = {_OutPutFileType.upper()}")
        root.submenu.entryconfig(2, label='Export to PDF')


def change_remove_meters():
    global _RemoveMeters
    if _RemoveMeters:
        _RemoveMeters = False
        _debug(f"Remove Meters {_RemoveMeters}")
...
def processFile(file_input):
    global _BackupPath
    global _RemoveMeters
    fileOutput = 'out_' + os.path.basename(file_input).replace(" ", "_")
    print(f"Opening {file_input}")
    with open(file_input, 'r') as f:
        ...
        with open(fileOutput, 'w') as g:
            ...
            for line in f:
                ...
                _debug(line)
                g.writelines(line)
            if not matches and _RemoveMeters:
                root.warning_box("Warning", "No matches found!")
        g.close()
    f.close()
    if _converter:
        try:
            print("Converting " + os.path.basename(file_input))
            fileGV = fileOutput[:-4] + '.gv'
            fileOut = f'{fileOutput[:-4]}.{_OutPutFileType}'
            cmd1 = '.\dot -Tgv {0} -o {1}'.format(fileOutput, fileGV)
            cmd3 = 'del {0} && del {1} > nul'.format(fileOutput, fileGV)
            _debug("Generating GV file...")
            os.system(cmd1)
            _debug(f"Generating {_OutPutFileType.upper()} file...")
            try:
                render('dot', _OutPutFileType, f".\\{fileGV}")
                os.rename(f".\\{fileGV}.{_OutPutFileType}", f".\\{fileOut}")
                _debug(f"Moving {fileOut} file to {os.path.dirname(file_input)}\\RPL Graph Out")
                smove(f".\\{fileOut}", os.path.dirname(file_input) + f"\\RPL Graph Out\\{fileOut}")
                _debug(
                    f"Moving {os.path.basename(file_input)} file to {os.path.dirname(file_input)}\\RPL Graph Proccessed")
                smove(file_input,
                      os.path.dirname(file_input) + f"\\RPL Graph Proccessed\\{os.path.basename(file_input)}")
                print(f"Successfully converted {os.path.basename(file_input)}\n")
            except ExecutableNotFound:
                _debug("Rendering not working...")
                print("Install Latest Graphviz MSI Executable.")
            _debug("Cleaning up base files...")
            os.system(cmd3)
        except:
            print("Conversion Crashed...")
    else:
        print(f"Converter Off: Saving {fileOutput} to {os.path.dirname(file_input)}\\RPL Graph Out")
        smove(f".\\{fileOutput}", os.path.dirname(file_input) + f"\\RPL Graph Out\\{fileOutput}")
...
def create_folders(path):
    if not os.path.exists(path+'\\RPL Graph Out'):
        print(f"Creating {path}+\\RPL Graph Out")
        os.makedirs(path+'\\RPL Graph Out')
    if not os.path.exists(path+'\\RPL Graph Proccessed'):
        print(f"Creating {path}+\\RPL Graph Proccessed")
...
def run():
    ...
    if os.path.isfile(file_or_dir):
        ...
        list_files = []
        for file in glob.glob(file_or_dir+"\\*.txt"):   #fixme: with os.path.join()
            _debug(file)
...
class Root(Tk):
    def __init__(self):
        ...
        row = 5
        pad = 6
        self.cb_medidores = Checkbutton(self, command=change_remove_meters, text="Remove Meters:", width=14)
        ...
        self.cb_backup = Checkbutton(self, command=change_backup_path, text="Show Backup Path")
        ...
        self.rb_onefile = Radiobutton(self, text="Pick a file.",
                                      variable=self.rb_var, value=1, command=toggle_one_file)  # command
        self.rb_allfiles = Radiobutton(self, text="Run on all files from folder.",
                                       variable=self.rb_var, value=0, command=toggle_one_file)
        ...
        self.submenu.add_command(label="Turn Debug On", command=change_debug_mode)
        self.submenu.add_command(label="Turn Converter Off", command=change_converter)
        self.submenu.add_command(label=f"Export to PDF", command=change_export_type)
