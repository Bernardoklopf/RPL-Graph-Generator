...
def processFile(file_input):
    ...
    if _converter:
        try:
            ...
            cmd32 = 'gswin32c -o {0} -sDEVICE=pngalpha {1} > nul'.format(filePNG, filePS) #Fixme: check whats wrong with GSwin
            cmd64 = 'gswin64c -o {0} -sDEVICE=pngalpha {1} > nul'.format(filePNG, filePS)
            ...
            _debug("Generating PS file...")
            os.system(cmd1)
            _debug("Generating PNG file...")
            try:
                _debug("Running GSWin32c")
                os.system(cmd32)
            except:
                _debug("Running GSWin32c")
                os.system(cmd64)
            _debug(f"Moving {filePNG} file to {os.path.dirname(file_input)}\\RPL Graph Out")
            smove(f".\\{filePNG}", os.path.dirname(file_input)+f"\\RPL Graph Out\\{filePNG}")
